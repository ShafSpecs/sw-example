/**
 * Inner APIs for the `strategy` module. The APIs in this file won't be typically 
 * imported by the dev.
 */
...
export class StrategyHandler {
  ...
  private readonly _pluginStateMap: Map<Plugin, CustomMapObject>;
   ...
   * @return {boolean}
   */
  hasCallback<C extends keyof Plugin>(name: C): boolean {
   ...
   */
  async runCallbacks<C extends keyof NonNullable<Plugin>>(
    name: C,
    param: Omit<PluginCallbackParam[C], 'state'>,
  ): Promise<void> {
    for (const callback of this.iterateCallbacks(name)) {
      // TODO(philipwalton): not sure why `any` is needed. It seems like
      // this should work with `as PluginCallbackParam[C]`.
   ...
   */
  *iterateCallbacks<C extends keyof Plugin>(
    name: C,
  ): Generator<NonNullable<Plugin[C]>> {
    for (const plugin of this._strategy.plugins) {
      if (typeof plugin[name] === 'function') {
        ...
        yield statefulCallback as NonNullable<Plugin[C]>;
      }
    }
  }
